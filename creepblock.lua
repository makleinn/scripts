...
local key = Menu.AddKeyOption({"Utility", "Creep Block"}, "CreepBlock KEY", Enum.ButtonCode.KEY_SPACE)
--local enemyHeroBlock = Menu.AddOptionBool({ "Utility", "Creep Block", "[auto-summon] EnemyHeroBlock" }, "Enabled", "Block enemy hero with summoned units.")
Blocker.skipRangedCreep = Menu.AddOptionBool({ "Utility", "Creep Block"},"Skip ranged creep", "Enabled", "Bot will try to skip ranged creep.")
...
function Blocker.OnDraw()
    ...
    if less_stopping then
        Renderer.SetDrawColor(0, 0, 0, 222)
        Renderer.DrawText(font, hx, hy, 'CREEP BLOCK (TOWER NEAR)', 1)
    end
    ...
    if best_position then
        ...
        -- if curtime > last_stop and dist >= 15 * speed / 325 and dist <= 150 * speed / 325 then
        if curtime > last_stop and dist >= 10 and dist <= speed/2 then
            last_stop = curtime + 0.24 * 325 / speed*1.6
            if less_stopping then
            ...
            -- end
			if speed > 435 then
			sleep = curtime + 0.15 * 325 / speed*1.4 else
			end
			if speed > 340 then
			sleep = curtime + 0.11 * 325 / speed*1.22 else
			end
			if speed < 340 then
            sleep = curtime + 0.08 * 325 / speed*1.11 else 
			end
